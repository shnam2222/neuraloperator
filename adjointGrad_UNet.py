...
from UNet_small_MSE import UNet
...
def train_UNet():
    ...
    # configurations all in config.py
    ...
    pattern_train =np.expand_dims(pattern_train, 1)
    gradient_train =np.expand_dims(gradient_train, 1)
    ...
    pattern_test =np.expand_dims(pattern_test, 1)
    gradient_test =np.expand_dims(gradient_test, 1)
    ...
    for ep in range(epochs):
        ...
        train_l2 /= n_train
        test_l2 /= n_test
        test_l2_array = np.append(test_l2_array, test_l2)
        t2 = default_timer()
    ...
    x = x.reshape(epochs)
    test_l2_array = test_l2_array.reshape(epochs)
    '''
    plt.plot(x, test_l2_array)
    plt.show()
    '''
    ...
    sample = sample_pattern.reshape(1,1,size)
    ...
    #filename = "UNet_"+"n_train_"+str(n_train)+"_epochs_"+str(epochs)+"_" + str(trial_no) + ".mat"
